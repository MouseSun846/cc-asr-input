# 语音转文字工具 (Audio Transcription Tool)

一款基于Python开发的实时语音转文字工具，支持全局快捷键控制、悬浮窗状态显示，以及自动复制粘贴功能。

## 功能特性

### 核心功能
- **全局快捷键控制**：按F9键开始录音，再次按F9键停止录音
- **实时录音**：使用系统音频设备录制高质量音频
- **语音转文字**：调用SenseVoice API将音频转换为文字
- **自动复制粘贴**：转写结果自动复制到剪贴板，并尝试粘贴到当前活动窗口

### 用户界面
- **悬浮窗显示**：录音时显示悬浮窗，实时展示录音状态
- **状态提示**：显示"录音中"、"转写中"等状态信息
- **动画效果**：转写过程中显示动态加载动画
- **拖动支持**：悬浮窗支持拖动到屏幕任意位置
- **智能隐藏**：转写完成后自动隐藏悬浮窗

### 技术特性
- **多线程处理**：录音、转写、UI更新并行处理，无卡顿
- **设备选择**：支持选择特定的音频输入设备
- **临时文件管理**：自动创建和清理临时音频文件
- **错误处理**：完善的异常处理和日志输出
- **后台运行**：支持最小化到后台，持续监听快捷键
